    ...
    <div class="bg-white/90 shadow-inner py-1">
        <nav class="flex justify-center space-x-6 text-gray-800">
            <?php foreach($menu as $item): ?>
                <?php 
                $isActive = ($item['url'] !== '#' && basename($item['url'], ".php") === $activePage) || 
                            (isset($item['children']) && in_array($activePage, array_map(fn($c)=>basename($c['url'], ".php"), $item['children'])));
                $activeClass = $isActive ? 'font-bold text-blue-600' : 'font-normal hover:text-blue-600';
                ?>
                <?php if(isset($item['children'])): ?>
                    <div class="relative group">
                        <a href="#" class="flex items-center gap-1 px-3 py-1 rounded <?= $activeClass ?>">
                            <i class="<?= $item['icon'] ?>"></i> <?= $item['label'] ?>
                        </a>
                        <ul class="absolute left-0 top-full mt-1 w-52 bg-white rounded-md shadow-lg py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                            <?php foreach($item['children'] as $child): ?>
                                <?php 
                                $childActive = basename($child['url'], ".php") === $activePage;
                                $childClass = $childActive ? 'font-bold text-blue-600' : 'font-normal hover:text-blue-600';
                                ?>
                                <li>
                                    <a href="<?= $child['url'] ?>" class="flex items-center gap-2 px-3 py-1 rounded <?= $childClass ?>">
                ...
                <?php else: ?>
                    <a href="<?= $item['url'] ?>" class="flex items-center gap-1 px-3 py-1 rounded <?= $activeClass ?>">
                        <i class="<?= $item['icon'] ?>"></i> <?= $item['label'] ?>
                    </a>
                <?php endif; ?>
            <?php endforeach; ?>
...
<div id="mobile-sidebar" class="fixed top-0 left-0 h-full w-64 bg-white shadow-lg transform -translate-x-full transition-transform duration-300 z-50">
    ...
    <ul class="flex flex-col p-4 space-y-1 text-gray-800">
        <?php foreach($menu as $item): ?>
            ...
            <?php if(isset($item['children'])): ?>
                <li>
                    <button class="flex items-center justify-between w-full py-2 px-3 rounded <?= $activeClass ?> servicios-toggle">
                        <span class="flex items-center gap-2"><i class="<?= $item['icon'] ?> w-5"></i> <?= $item['label'] ?></span>
                        <svg class="w-4 h-4 transition-transform transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                    <ul class="sub-menu overflow-hidden max-h-0 flex-col pl-6 mt-1 space-y-1 transition-all duration-300">
                        <?php foreach($item['children'] as $child): ?>
                            <?php 
                            $childActive = basename($child['url'], ".php") === $activePage;
                            $childClass = $childActive ? 'font-bold text-blue-600' : 'font-normal hover:text-blue-600';
                            ?>
                            <li>
                                <a href="<?= $child['url'] ?>" class="flex items-center gap-2 py-1 px-3 rounded <?= $childClass ?>">
            ...
            <?php else: ?>
                <li>
                    <a href="<?= $item['url'] ?>" class="flex items-center gap-2 py-2 px-3 rounded <?= $activeClass ?>">
